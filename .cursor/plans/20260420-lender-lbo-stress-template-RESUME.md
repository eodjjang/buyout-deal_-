...
**마지막 갱신**: 2026-04-21 Task 17 완료 (`test_integrity.py` 7 E2E tests; impl `fc83825` + polish `b45e058` docstring/가드; spec ✅ + code ✅ APPROVED)
...
```
- `test_bootstrap.py` — 3
- `test_conventions.py` — 5 (Task 16 선행 리팩토에서 `define_name` 테스트 +1)
- `test_s0_readme.py` — 3
- `test_s1_input.py` — 9
...
**없음.** HEAD `b45e058` 기준 워크트리 클린·`pytest` 71 passed.

---

## 5. 다음 액션 — Wave 5: Task 18 (README + v0.5 태그)

### 개요

- **위치**: 플랜 §Task 18 (라인 3233~3278).
- **Files**: `README.md` (워크트리 루트) — 설치·빌드·테스트·수기 검증(플랜 Task 17 Step 3 10단계 복붙)·v0.6 로드맵·디자인·플랜 링크.
- **의존**: HEAD `b45e058` (Task 17 완료).
- **Step 2**: `git tag -a v0.5.0 -m "LBO Stress Template v0.5 — MVP with all 13 sheets, Preconditions resolved"` (플랜 그대로).
- **검증**: `pytest -v` → 71 passed 유지.

### Task 18 implementer 힌트

- **HEAD**: `b45e058`.
- 플랜 Task 18 전문을 implementer 프롬프트에 붙여넣기. **작업 디렉터리**: `C:\vibecoding\works\buyout-deal-lbo-impl`.
- Final 전체 `code-reviewer` 서브에이전트는 Task 18 완료 후 `RESUME` §6 참조.
- **수기 Excel(Task 17 Step 3)**: 아직 안 했다면 `python -m lbo_template.build --output dist/LBO_Stress_Template_v0.5.xlsx` 후 플랜 10단계 병행 가능.
...
git log --oneline -8                             # b45e058, fc83825, 7d54ec5, …
git status --short                               # (empty — clean tree)
python -m pytest tests/ -v                       # 71 passed
```

기대 출력 요약:
- HEAD = `b45e058 test(integrity): clarify self-ref heuristic docstring + guard Sources-Uses formula type`
- pytest tail: `tests/test_integrity.py::… PASSED` + `71 passed in … s`

모두 정상이면 §5 Wave 5 — Task 18 (README + tag) 착수. 수기 Excel 검증(Task 17 Step 3)은 아직 안 한 경우 병행 가능.
