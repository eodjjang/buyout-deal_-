...
**마지막 갱신**: 2026-04-21 Task 15 완료 (`7_Returns_LTV` Method 1/2/3 추상화 + 9-col LTV + Applied_*/DCF_* 링크; impl `4d99510` → polish `ece0f88`; spec reviewer ✅ + code reviewer APPROVED with polish; M1/M6 반영)
...
## § 자동 저장 / 핸드오프 프로토콜 (Task 6 ~ Task 15)
...
**없음.** HEAD `ece0f88` 기준 워크트리 클린·`pytest` 56 passed.

---

## 5. 다음 액션 — Wave 4: Task 16 (`8_Dashboard`)

### 개요

- **위치**: 플랜 §Task 16 (라인 2864~ around 3200). 설계 §8 v0.5.
- **Files** (플랜): `src/lbo_template/sheets/s8_dashboard.py`(스텁 → 본격 구현), **신규** `tests/test_s8_dashboard.py`.
- **의존**: **전 시트 대부분** — T5(`Case_Switch`는 사실 T6에서 생성)·T9(`Dividend_Row`/`Holdco_ICR_Row`/`Opco_ICR_Row`/`Opco_DSCR_Row`·KPI named ranges)·T10(`DCF_EV`/`DCF_Equity_Value`)·T15(`DASH_Valuation_Method{i}_*`, `DASH_LTV_Method{i}_*`). T16은 이들을 dashboard로 집약.
- **신규 Named Ranges (~60개)**:
  - `DASH_Case`, `DASH_Version` (scalar 2)
  - `DASH_DSCR_Min`, `DASH_ICR_Opco_Min`, `DASH_ICR_Holdco_Min` (scalar 3)
  - `DASH_Div_FY1` ~ `DASH_Div_FY5` (5)
  - `DASH_Lev_NetLeverage_FY1` ~ `DASH_Lev_NetLeverage_FY5` (5)
  - `DASH_CFTable_Row1_FY1` ~ `DASH_CFTable_Row8_FY5` (40) + `DASH_CFTable_Row1_Label` ~ `Row8_Label` (8)
- **검증**: `pytest tests/ -v` 전부 통과 (예상 60 passed = 56 + 4 신규). 커밋 메시지: `feat(s8_dashboard): unified 5-table summary + DASH_* named range cluster + CFTable 8x5`.

### Task 16 implementer 힌트

- **HEAD**: `ece0f88` (Task 15 polish 완료).
- 플랜 Task 16 Step 1(tests 4개) + Step 2(builder 본문) **전문**을 implementer 프롬프트에 붙여넣기. Step 2는 플랜에서 ~240행. **5개 표 + CFTable 8×5 + scalar + scenario meta 순서 그대로 구현**.
- **사전 점검 (T5/T6/T9 교훈)**: implementer 프롬프트 작성 시 아래 named range가 실제 존재하는지 컨트롤러가 재확인:
  - `Case_Switch` (T6, `2_Stress_Panel!B3`)
  - `Dividend_Row`, `Holdco_ICR_Row`, `Opco_ICR_Row`, `Opco_DSCR_Row`, `Net_Leverage_Row` (T9, `5_CF_Waterfall` KPI 행들) — **`5_CF_Waterfall` 실제 수식에서 named range 이름이 위 Dashboard 수식이 쓰는 이름과 일치하는지** 확인. 불일치면 T16 implementer 프롬프트에 교정 주입.
  - `DASH_Valuation_Method{i}_{Label,Multiple,EV}`, `DASH_LTV_Method{i}_{Opco,Cumulative}` (T15 ✓ 확정).
- **컨트롤러 판단 (T15 Deferred Nit M3)**: T16이 `DASH_*` ~60개 등록 예정. **T16 착수 전에** `conventions.py`에 `define_name(wb, name, ref)` 2-line 헬퍼를 먼저 추가하는 **사전 리팩토 커밋 1개**를 선행할 것 (예: `chore(conventions): add define_name helper + migrate s7 to use it`). T16 implementer가 이 헬퍼를 사용하면 60회 × 3행 → 60회 × 1행 코드 절감. 판단은 컨트롤러 몫 — 사용자 물음 없이.
- **분량 주의**: Task 16은 본 프로젝트에서 가장 큰 단일 Task. spec reviewer·code reviewer 프롬프트에 "60개 named range 전수 검증 필수" 명시. 샌드박스 shell 이슈 대비 git ref fallback 지시.
- **Task 14 학습**: Task 14는 컨트롤러 직행(플랜 ~180행)을 감행하여 spec/code 리뷰가 누락됨. Task 16은 Task 14보다 크지만 implementer에게 위임 + 3단 리뷰 루프 준수가 품질상 이득.
...
| W4 | **T16 8_Dashboard** | **⏳ 다음** | 전 시트 의존. ~60 DASH_* named range. **`define_name` 헬퍼 선행 리팩토 권장 (M3)** |
...
git log --oneline -8                             # ece0f88, 4d99510, 0f4a712, 97a521d, cee5eab, …
git status --short                               # (empty — clean tree)
python -m pytest tests/ -v                       # 56 passed
```

기대 출력 요약:
- HEAD = `ece0f88 chore(s7_returns_ltv): polish row-range comments + add anchor/row-formula contract tests`
- pytest tail: `tests/test_s9_ciq_and_peer.py::test_three_year_average_of_average PASSED [100%]` + `56 passed in 0.X s`

모두 정상이면 §5 Wave 4 — Task 16 (`8_Dashboard`) 착수. (선행 리팩토 `define_name` 헬퍼 도입 여부는 컨트롤러 판단)
